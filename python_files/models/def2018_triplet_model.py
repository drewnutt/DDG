...
class Net(nn.Module):
        def __init__(self, dims,config):
                ...
                if config.hidden_size > 0:
                    ...
                    self.ddg = nn.Linear(last_size,1,bias=False) #the bias should be 0
                    self.add_module('DDG',self.ddg)
        ...
        def forward(self,x1,x2,x3):
                ...
                lig3 = self.forward_one(x3)
                lig1_aff = self.dg(lig1)
                lig2_aff = self.dg(lig2)
                lig3_aff = self.dg(lig3)
                diff1 = self.dropout(lig1 - lig2)
                diff2 = self.dropout(lig2 - lig3)
                diff3 = self.dropout(lig3 - lig1)
                return self.ddg(diff1), self.ddg(diff2), self.ddg(diff3), lig1_aff, lig2_aff, lig3_aff, lig1, lig2, lig3
