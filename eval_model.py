...
parser.add_argument('--eval_data_lig','-el',required=True,help='lig cache data to evaluate the trained model on')
parser.add_argument('--eval_data_rec','-er',required=True,help='rec cache data to evaluate the trained model on')
...
def test(model, teste, size):
	...
	with torch.no_grad():
		for _ in range(size[0]):	
			...
			batch_1.extract_label(1, float_labels)
			labels = torch.unsqueeze(float_labels,1).float().to('cuda')
			output = model(input_tensor[:,:28,:,:,:],input_tensor[:,28:,:,:,:])
			loss = criterion(output,labels)
			test_loss += loss
			output_dist += output.flatten().tolist()
			actual += labels.flatten().tolist()
	...
	r = pearsonr(np.array(actual),np.array(output_dist))
	return test_loss/(size[2]), output_dist,r[0], actual
...
dims = gmaker.grid_dimensions(14*4) #only one rec+one lig provided to each leg of network
...
tr_loss, out_dist, r, actual_dist= test(model, teste, tt_nums)
