...
def getDDGstats(compare_ddg_df,ground_truth_files):
    ...
    comp_dict = dict()
    file_data = pd.read_csv(ground_truth_files[0], sep=' ', header=None)
    train_data = file_data[[1,file_data.columns[-3],file_data.columns[-2],file_data.columns[-1]]].copy()
    train_data.columns = ['DDG','rec','lig1','lig2']
    ...
    fig = plt.figure()
    rvals, recvals = [],[]
    for idx, (rec, group) in enumerate(train_data.groupby('rec')):
        sort_group = group.sort_values(by=['lig1_lig2'])
        subset_gnina_ddg = compare_ddg_df[compare_ddg_df['lig1_lig2'].isin(group['lig1_lig2'])]

        if subset_gnina_ddg.shape[0] == group.shape[0]:
            if group.shape[0] > 1:
                pass
            else:
                print(f"{rec} len = 1")
        ...
        for col in subset_gnina_ddg.columns:
            if 'CNNaff' not in col:
                continue
            r,_ = pearsonr(subset_gnina_ddg[col].to_numpy(),sort_group['DDG'].to_numpy())
            rmse = np.sqrt(((subset_gnina_ddg[col].to_numpy()-sort_group['DDG'].to_numpy()) ** 2).mean())
        print(f"{rec}: R={r},RMSE={rmse}")
        rvals.append(r)
        recvals.append(rec)
    print(len(rvals))
    print(len([r for r in rvals if r > 0.3]))
    plt.bar(range(len(rvals)),rvals,label=recvals)
    plt.ylabel("Pearson's R")
    plt.savefig('for_gnina.png',dpi=500)
